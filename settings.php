    ...
    <form method="POST" action="settings.php" enctype="multipart/form-data" class="flex-1 flex flex-col overflow-hidden" <?php echo $isAdmin ? '' : 'onsubmit="return false;"'; ?>>
        ...
        <div class="flex-1 overflow-hidden flex relative z-10">
            ...
            <div class="flex-1 overflow-y-auto p-8 relative scroll-smooth" id="settings-content-area">
                ...
                <div id="tab-content-annual_tips" class="tab-content hidden space-y-6 max-w-4xl mx-auto animate-fade-in">
                    <div class="bg-dark-surface/60 backdrop-blur-md border border-white/5 rounded-2xl p-8 glass-panel">
                        <h3 class="text-xl font-bold text-white flex items-center gap-3 mb-6">
                            <span class="material-icons-round text-yellow-500">tips_and_updates</span>
                            <?php echo __('yearly_tips_title'); ?>
                        </h3>

                        <!-- Years Slider -->
                        <div class="mb-8">
                            <label class="text-xs font-bold text-gray-400 mb-3 block"><?php echo __('select_year_tip_placeholder'); ?></label>
                            <div id="archived-years-container" class="flex gap-3 overflow-x-auto pb-4 custom-scrollbar snap-x">
                                <!-- Dynamic Years -->
                                <div class="w-full text-center py-8 text-gray-500 text-sm italic">
                                    <?php echo __('loading_text'); ?>
                                </div>
                            </div>
                        </div>

                        <!-- Advice Content -->
                        <div id="archived-advice-display" class="hidden">
                             <!-- Reusing the structure from reports.php -->
                             <div class="relative group">
                                <div id="setting-advice-glow" class="absolute -inset-1 bg-gradient-to-r from-yellow-400 via-yellow-200 to-yellow-400 rounded-2xl blur opacity-25 transition duration-1000 animate-pulse"></div>
                                <div id="setting-advice-card" class="glass-card p-8 relative rounded-2xl border border-yellow-500/30 bg-dark-surface/90 backdrop-blur-xl">
                                    <div class="flex flex-col md:flex-row gap-8 items-start">
                                        ...
                                        <div class="w-full md:w-1/3 flex flex-col items-center justify-center text-center border-b md:border-b-0 md:border-l border-white/10 pb-6 md:pb-0 md:pl-6">
                                            <div id="setting-score-circle" class="w-24 h-24 rounded-full border-4 border-yellow-500/50 flex items-center justify-center relative mb-4 shadow-[0_0_30px_rgba(234,179,8,0.2)]">
                                                <div id="setting-score-spinner" class="absolute inset-0 rounded-full border-4 border-t-yellow-400 animate-spin" style="animation-duration: 3s;"></div>
                                                <span id="setting-financial-score" class="text-3xl font-bold text-white drop-shadow-md">--</span>
                                            </div>
                                            <h3 id="setting-score-title" class="text-lg font-bold text-yellow-400 mb-1"><?php echo __('financial_health_score'); ?></h3>
                                            <div id="setting-financial-verdict" class="text-xs text-gray-300 font-medium px-4 py-1.5 bg-white/5 rounded-full mt-2 border border-white/10">--</div>
                                        </div>
                                        
                                        <!-- Right: Detailed Advice -->
                                        <div class="w-full md:w-2/3">
                                            <h3 class="text-xl font-bold text-white mb-4 flex items-center gap-3 border-b border-white/10 pb-3">
                                                <span id="setting-advice-icon" class="material-icons-round text-yellow-400 text-2xl">psychology</span>
                                                <span id="setting-advice-year-title">...</span>
                                            </h3>
                                            <div id="setting-advice-list" class="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                                                <!-- Content -->
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div id="no-tips-placeholder" class="hidden text-center py-12 bg-white/5 rounded-2xl border border-white/5 border-dashed">
                            <span class="material-icons-round text-4xl text-gray-600 mb-2">history_toggle_off</span>
                            <p class="text-gray-500 text-sm"><?php echo __('no_archived_tips'); ?></p>
                        </div>
...
<script>
    ...
    function switchTab(tabName) {
        ...
        if(tabName === 'annual_tips') loadArchivedYears();
    }

    // Annual Tips Functions
    async function loadArchivedYears() {
        const container = document.getElementById('archived-years-container');
        container.innerHTML = '<div class="w-full text-center py-4 text-gray-500"><span class="material-icons-round animate-spin">sync</span> ' + window.__('loading_text') + '</div>';
        
        try {
            const res = await fetch('api.php?action=get_archived_years');
            const data = await res.json();
            ...
            if (data.success && data.data.length > 0) {
                ...
                data.data.forEach((year, index) => {
                    html += `
                    <div onclick="loadYearTip(${year}, this)" class="year-slide cursor-pointer flex-shrink-0 w-32 h-24 rounded-2xl bg-white/5 border border-white/10 hover:border-primary/50 hover:bg-primary/10 transition-all flex flex-col items-center justify-center gap-1 group snap-center relative overflow-hidden">
                        <span class="text-2xl font-bold text-white group-hover:text-primary transition-colors z-10">${year}</span>
                        <span class="text-[10px] text-gray-500 group-hover:text-primary/70 z-10">${window.__('click_to_view_details')}</span>
                        <div class="absolute inset-0 bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    </div>`;
                });
                container.innerHTML = html;
                document.getElementById('no-tips-placeholder').classList.add('hidden');
                document.getElementById('archived-advice-display').classList.add('hidden'); 
            } else {
                container.innerHTML = '';
                document.getElementById('no-tips-placeholder').classList.remove('hidden');
                document.getElementById('archived-advice-display').classList.add('hidden');
            }
        } catch (e) {
            container.innerHTML = '<div class="text-red-500 text-sm p-4 w-full text-center">' + window.__('error_occurred') + '</div>';
        }
    }

    async function loadYearTip(year, el) {
        document.querySelectorAll('.year-slide').forEach(d => {
            d.classList.remove('ring-2', 'ring-primary', 'bg-primary/20');
            d.classList.add('bg-white/5');
        });
        
        if(el) {
            el.classList.remove('bg-white/5');
            el.classList.add('ring-2', 'ring-primary', 'bg-primary/20');
        }

        const display = document.getElementById('archived-advice-display');
        display.classList.remove('hidden');
        display.classList.add('opacity-50', 'pointer-events-none');
        
        try {
            const res = await fetch(`api.php?action=get_yearly_advice&year=${year}`);
            const result = await res.json();
            
            if (result.success) {
                renderSettingsAdvice(result.data);
                // Scroll to display
                display.scrollIntoView({ behavior: 'smooth', block: 'start' });
            }
        } catch(e) {
            showToast(window.__('error_occurred'), false);
        } finally {
            display.classList.remove('opacity-50', 'pointer-events-none');
        }
    }

    let settingsScoreInterval = null;

    function renderSettingsAdvice(data) {
        const adviceList = document.getElementById('setting-advice-list');
        const scoreEl = document.getElementById('setting-financial-score');
        const verdictEl = document.getElementById('setting-financial-verdict');
        const yearTitle = document.getElementById('setting-advice-year-title');
        
        yearTitle.innerText = data.year;
        ...
        if (settingsScoreInterval) clearInterval(settingsScoreInterval);
        ...
        // Apply Theme
        const adviceGlow = document.getElementById('setting-advice-glow');
        const adviceCard = document.getElementById('setting-advice-card');
        const scoreCircle = document.getElementById('setting-score-circle');
        const scoreSpinner = document.getElementById('setting-score-spinner');
        const scoreTitle = document.getElementById('setting-score-title');
        const adviceIcon = document.getElementById('setting-advice-icon');

        adviceGlow.className = `absolute -inset-1 bg-gradient-to-r ${theme.glow} rounded-2xl blur opacity-25 transition duration-1000 animate-pulse`;
        
        adviceCard.classList.remove('border-yellow-500/30', 'border-red-500/30', 'border-orange-500/30', 'border-green-500/30');
        adviceCard.classList.add(theme.bg_border);

        scoreCircle.classList.remove('border-yellow-500/50', 'border-red-500/50', 'border-orange-500/50', 'border-green-500/50');
        scoreCircle.classList.add(`${theme.border}/50`);
        scoreCircle.style.boxShadow = `0 0 30px ${theme.shadow}`;

        scoreSpinner.classList.remove('border-t-yellow-400', 'border-t-red-400', 'border-t-orange-400', 'border-t-green-400');
        scoreSpinner.classList.add(theme.spinner_border);

        scoreTitle.className = `text-lg font-bold ${theme.text_light} mb-1`;
        adviceIcon.className = `material-icons-round ${theme.text_light} text-2xl`;

        settingsScoreInterval = setInterval(() => {
            if (currentScore >= targetScore) {
                clearInterval(settingsScoreInterval);
                scoreEl.textContent = targetScore + '/10';
                scoreEl.className = `text-3xl font-bold drop-shadow-md ${theme.text}`;
            } else {
                currentScore++;
